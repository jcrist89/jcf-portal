...
export function ProgramStructureEditor({
  ...
  return (
    <div>
      <Link href={backHref} className="text-jcf-gray text-xs uppercase tracking-widest hover:text-jcf-gold">
        {backLabel}
      </Link>

      <div className="mt-2 mb-6 flex flex-col gap-3">
        {showName && <Input label="Program Name" value={name} onChange={(e) => setName(e.target.value)} />}
