...
export function OnboardingForm({ existingGoal }: { existingGoal: string | null }) {
  const router = useRouter();
  const needsGoalStep = !existingGoal;
  const totalSteps = needsGoalStep ? 3 : 2;
  ...
  const [goal, setGoal] = useState<string | null>(existingGoal);
      ...
      <div className="w-full max-w-md bg-jcf-panel border border-white/10 rounded-sm p-6">
        ...
        {step === 1 && (
          <div className="flex flex-col gap-4">
            ...
            {error && !needsGoalStep && <p className="text-jcf-danger text-sm">{error}</p>}
